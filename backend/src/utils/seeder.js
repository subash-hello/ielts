...
const seedDatabaseIfEmpty = async () => {
  try {
    ...
    for (let i = 1; i <= 30; i++) {
      ...
      const cambridgeTestId = ((i - 1) % 10 + 1).toString();
      const cambridgeTest = cambridgeListeningTests[cambridgeTestId] || cambridgeListeningTests['1'];
      const actualParts = cambridgeTest.parts;
      const listeningParts = actualParts.map(p => ({
        part: p.part,
        title: p.title,
        type: p.type,
        audioUrl: p.audioUrl,
        ...
        questions: p.questions.map((q, idx) => ({
          ...
          text: q.text,
          options: q.options || (q.type === 'trueFalseNotGiven' ? ["True", "False", "Not Given"] : []),
          correctAnswer: q.correctAnswer || q.correct || '',
          mapImage: q.mapImage || null
        }))
      }));
